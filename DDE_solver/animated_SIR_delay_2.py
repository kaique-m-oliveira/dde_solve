...
# --- Sliders setup ---
# Adjust bottom margin to accommodate many sliders, moving them lower
fig.subplots_adjust(left=0.1, bottom=0.45, right=0.9, top=0.9)  # Adjusted bottom margin

# Define common slider height and spacing
slider_height = 0.02
slider_spacing = 0.035

# Sliders for _BETA, _PSI, _TAU (adjusted positions)
slider_row_start_bottom = 0.40  # New starting position for the top row of sliders

ax_beta = fig.add_axes([0.1, slider_row_start_bottom, 0.35, slider_height])
beta_slider = Slider(
    ax=ax_beta, label="Beta (β)", valmin=0.01, valmax=0.5, valinit=_BETA
)

ax_psi = fig.add_axes(
    [0.1, slider_row_start_bottom - slider_spacing, 0.35, slider_height]
)
psi_slider = Slider(ax=ax_psi, label="Psi (ψ)", valmin=0.01, valmax=0.5, valinit=_PSI)

ax_tau = fig.add_axes(
    [0.1, slider_row_start_bottom - 2 * slider_spacing, 0.35, slider_height]
)
tau_slider = Slider(ax=ax_tau, label="Tau (τ)", valmin=0.0, valmax=20.0, valinit=_TAU)

# NEW SLIDERS for the _FIXED parameters (adjusted positions)
# Lambda (Λ)
ax_lambda = fig.add_axes(
    [0.1, slider_row_start_bottom - 3 * slider_spacing, 0.35, slider_height]
)
...
# A
ax_A = fig.add_axes(
    [0.55, slider_row_start_bottom, 0.35, slider_height]
)  # Start new column
A_slider = Slider(ax=ax_A, label="A", valmin=0.0, valmax=50.0, valinit=_A, valstep=1.0)

# Mu (μ)
ax_mu = fig.add_axes(
    [0.55, slider_row_start_bottom - slider_spacing, 0.35, slider_height]
)
mu_slider = Slider(ax=ax_mu, label="Mu (μ)", valmin=0.01, valmax=0.5, valinit=_MU)

# P
ax_P = fig.add_axes(
    [0.55, slider_row_start_bottom - 2 * slider_spacing, 0.35, slider_height]
)
p_slider = Slider(ax=ax_P, label="P", valmin=0.0, valmax=1.0, valinit=_P)

# Alpha (α)
ax_alpha = fig.add_axes(
    [0.55, slider_row_start_bottom - 3 * slider_spacing, 0.35, slider_height]
)
alpha_slider = Slider(
    ax=ax_alpha, label="Alpha (α)", valmin=0.01, valmax=0.5, valinit=_ALPHA
)

# Theta (θ)
ax_theta = fig.add_axes(
    [0.1, slider_row_start_bottom - 4 * slider_spacing, 0.35, slider_height]
)  # Continue first column
theta_slider = Slider(
    ax=ax_theta, label="Theta (θ)", valmin=0.0, valmax=0.2, valinit=_THETA
)

# Sigma (σ)
ax_sigma = fig.add_axes(
    [0.55, slider_row_start_bottom - 4 * slider_spacing, 0.35, slider_height]
)  # Continue second column
sigma_slider = Slider(
    ax=ax_sigma, label="Sigma (σ)", valmin=0.0, valmax=0.5, valinit=_SIGMA
)


# Sliders for t_end and h_step (adjusted positions)
ax_tend = fig.add_axes(
    [0.1, slider_row_start_bottom - 5 * slider_spacing, 0.35, slider_height]
)
tend_slider = Slider(
    ax=ax_tend,
    label="t_end",
...
ax_hstep = fig.add_axes(
    [0.55, slider_row_start_bottom - 5 * slider_spacing, 0.35, slider_height]
)
hstep_slider = Slider(
    ax=ax_hstep,
    label="h_step",
