...
def run_dde_test_with_random_check(
    ...
    try:
        ...
        # Iterate through each interval stored in history_data
        for i in range(len(history_data) - 1):
            ...
            for _ in range(num_random_points_per_interval):
                # Generate random theta strictly between 0 and 1 (to avoid hitting nodes exactly)
                theta = random.uniform(1e-6, 1.0 - 1e-6)
                t_eval_random = t_k + theta * h_interval

                # Interpolate using the cubic_hermite function
...
if __name__ == "__main__":
    # Test for the system DDE (y'(t) = -y(t - pi/2))
    run_dde_test_with_random_check(
        ...
        h_step=0.1,
        h_disc_guess=0.01,
        tolerance=1e-4,  # RK4 expected accuracy is good here
        num_random_points_per_interval=10,  # Check 10 random points within each step
    )
