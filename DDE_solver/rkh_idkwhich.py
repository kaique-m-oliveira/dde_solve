...
class OneStep:
    ...
    def uni_local_error_satisfied(self):
        ...
        max_uni_difference = (
            self.h / (32 * abs(2 * self.params.theta1 - 1))
            * np.linalg.norm(
                ((2 * self.params.theta1 - 1) / self.params.theta1) * self.K[0]
                - (2 * self.K[1] + 2 * self.K[2] + self.K[3])
                + (3 * self.params.theta1 - 2) *
                self.K[4] / (self.params.theta1 - 1)
...
class Solver:
    ...
    def solve_dde(self):
        t, h, tf = self.t_span[0], (self.params.TOL **
                                    (1 / 4)) * 0.1, self.t_span[-1]
        print("-" * 80)
        print("value h", h)
        print("-" * 80)
...
#
# t0, tf = solver.t[-2], solver.t[-1]
# x = np.linspace(t0, tf, 100)
# sin = np.sin(x)
# sol = np.array([solver.etas[-1](i) for i in x])
# print("min", min(abs(sol - sin)), "max", max(abs(sol - sin)))
#
# plt.plot(x, sin, color="red")
# plt.plot(x, sol, color="blue")
