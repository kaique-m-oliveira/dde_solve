...
class OneStep:
    ...
    def one_step_RK4(self):
        ...
        self.K[1] = f(tn + 0.5 * h, yn + 0.5 * h *
                      self.K[0], eta(alpha(tn + 0.5 * h)))
        self.K[2] = f(tn + 0.5 * h, yn + 0.5 * h *
                      self.K[1], eta(alpha(tn + 0.5 * h)))
    ...
    def test_one_step_RK4(self):
        ...
        print('eta atras', eta(alpha(tn)))
        print('eta - sen', abs(np.sin(alpha(tn)) - eta(alpha(tn))))
        print('y1 - sen(t1)', abs(np.sin(tn + h) - self.y[1]))
    ...
    def test_error_est(self):
        ...
        f, eta, alpha = self.solver.f, self.solver.eta, self.solver.alpha
        print('-'*80)
        print('test disc error')
        print('tn', tn)
        print('y_tilde - sen(t1)', abs(np.sin(tn + h) - self.y_tilde))
        print('K0, ..., K5', self.K[0], self.K[1],
              self.K[2], self.K[3], self.K[4], self.K[5])

    def disc_local_error_satistied(self):
        self.disc_local_error = np.linalg.norm(
            self.y_tilde - self.y[1]) / self.h
        # Handle zero error
        if self.disc_local_error <= self.params.TOL or abs(self.disc_local_error) < 1e-15:
            return True
        else:
            self.h = (
                max(
    ...
    def uni_local_error_satisfied(self):
        ...
        theta1 = self.params.theta1
        # max_uni_difference = (
        #     (self.h / (32 * abs(2 * theta1 - 1)))
        #     * np.linalg.norm(
        #         ((2 * theta1 - 1) / theta1) * self.K[0]
        #         - (2 * self.K[1] + 2 * self.K[2] + self.K[3])
        #         + (3 * theta1 - 2) *
        #         self.K[4] / (theta1 - 1)
        #         + self.K[5] / (theta1 * (theta1 - 1))
        #     )
        # )
        max_uni_difference = abs(self.eta[0](1/2) - self.eta[1](1/2))
    ...
    def test_uni_local_error_satisfied(self):
        tn, h, yn = self.t[0], self.h, self.y[0]
        f, eta, alpha = self.solver.f, self.solver.eta, self.solver.alpha
        print('-'*80)
        print('test_uni_local_error_satisfied')
        print('h', self.h, 'theta1', self.params.theta1)
        print('K0, ..., K5', self.K[0], self.K[1],
              self.K[2], self.K[3], self.K[4], self.K[5])
        print('error:', self.uni_local_error)
    ...
    def try_step_CRK(self):
        ...
        # WARN: removing for now
        # _disc_local_error_satistied = self.disc_local_error_satistied()
        # _uni_local_error_satisfied = self.uni_local_error_satisfied()
        # self.test_uni_local_error_satisfied()
        # print('='*80)
        # print('disc local error satisfied?', _disc_local_error_satistied)
        # print('uni local error satisfied?', _uni_local_error_satisfied)
        # print('='*80)
        #
    ...
    def one_step_CRK(self, max_iter=30):
        ...
        raise RuntimeError(
            f"Max iterations reached for step at tn = {
                self.t} and h = {self.h}"
        )
...
# step = solver.steps[-1]
#
# print('step.solver', step.solver)
# print('step.t', step.t)
# print('step.h', step.h)
# print('step.h_next', step.h_next)
# print('step.y', step.y)
# print('step.y_tilde', step.y_tilde)
# print('step.K', step.K)
# print('step.eta', step.eta)
# print('step.disc_local_error', step.disc_local_error)
# print('step.uni_local_error', step.uni_local_error)
# print('step.params', step.params)
#
# t0, tf = step.t[0], step.t[1]
#
#
# x = np.linspace(t0, tf, 100)
# sin = np.sin(x)
# print('Sol discreta', abs(np.sin(tf) - step.y[-1]))
# sol = np.array([step.eta[0](i) for i in x])
# print("min", min(abs(sol - sin)), "max", max(abs(sol - sin)))
# # print([(sol[i] - sol[i+1]) for i in range(len(sol) - 1)])
#
# plt.plot(x, sin, color="red")
# plt.plot(x, sol, color="blue")
# plt.show()
...
# print('solver t', solver.t, type(solver.t))
# print('solver y', solver.y, type(solver.y))
# print('solver etas', solver.etas, type(solver.etas))
# print('solver steps', solver.steps, type(solver.steps))
#
#
# print('testing first step accuracy')
# print('y - sin', abs(np.sin(solver.t[-1]) - solver.y[-1]))
